...
if ( ! function_exists( 'foundationpress_scripts' ) ) :
	function foundationpress_scripts() {
		...
		if(!is_front_page() ) {
			...
			wp_enqueue_style( 'gfonts', 'https://fonts.googleapis.com/css?family=Oswald:400|Open+Sans:300', array(), '1.1.0', 'all' );
		...
		#issue with init external js with parameters 
		#script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyDKBcKfHgMCOzkVGlKXQWFR1X3TMCsLcIA&libraries=places&callback=initMap" async defer></script>
		...
		$d = array(
			'asset_path' => get_stylesheet_directory_uri().'/dist/assets/images/',
			'site_name' => get_bloginfo('name')
		);
		wp_localize_script( 'foundation', 'edge', $d );
